...
ax.plot(u_veg_high, z, label='VEGGIE High Fan', color='#008080', lw=1.6)
ax.plot(u_veg_low, z, label='VEGGIE Low Fan (Draft)', color='#008080', linestyle='--', lw=1.6)
ax.plot(u_cara, z, label='CARA Dish Draft', color='#7570b3', lw=1.6)
...
# Flat plate boundary layer laminar/turbulent scaling
g_bl_model = 0.04 + 1.4 * np.sqrt(u_sweep)
ax.plot(u_sweep, g_bl_model, color='#111111', lw=1.5, label='Theory ($g_{bl} \propto \sqrt{U}$)')
ax.scatter([0.60, 1.50], [1.071, 1.745], color='#005696', s=45, label='APH (Nom / High)', zorder=5)
ax.scatter([0.065, 0.150], [0.219, 0.515], color='#008080', s=45, label='VEGGIE (Low / High)', zorder=5)
ax.scatter([0.082], [0.326], color='#7570b3', s=45, label='CARA (+Light)', zorder=5)
ax.scatter([0.005], [0.031], color='#d95f02', s=45, label='CHROMEX (Creeping)', zorder=5)
ax.axhline(0.25, color='#c70039', linestyle='--', lw=1.0, label='Stagnation Hypoxia Threshold')
...
ax = axs[1, 0]
ax.set_title('c  Canopy Stagnant Volume Fraction ($U < 0.05$ m/s)', loc='left', fontweight='bold')
sys = ['VEGGIE\n(Low)', 'VEGGIE\n(High)', 'APH\n(Nominal)', 'CARA\n(+Light)', 'CHROMEX\n(Sealed)', 'BRIC-LED\n(PDFU)']
stag = [52.8, 15.4, 2.6, 38.2, 100.0, 92.0]
c_list = ['#d73027', '#fee08b', '#1a9850', '#fee08b', '#d73027', '#d73027']
bars = ax.bar(range(6), stag, color=c_list, edgecolor='#333333', linewidth=0.6, width=0.6)
...
bars_t = ax.bar(range(6), tau, color=['#008080', '#008080', '#005696', '#7570b3', '#d95f02', '#e7298a'], edgecolor='#333333', linewidth=0.6, width=0.6)
ax.axhline(50.0, color='#c70039', linestyle='--', label='Leaf Damage Threshold (50 mPa)')
ax.set_xticks(range(6))
ax.set_xticklabels(sys, rotation=20, ha='right', fontsize=6.5)
ax.set_ylabel('Wall Shear Stress $\tau_w$ (mPa)')
ax.set_ylim(0, 55)
ax.grid(axis='y', linestyle=':', alpha=0.6)
ax.legend(loc='upper right')
for b, val in zip(bars_t, tau):
    ax.text(b.get_x() + b.get_width()/2, val + 1, f'{val:.1f}', ha='center', va='bottom', fontsize=6, fontweight='bold')
