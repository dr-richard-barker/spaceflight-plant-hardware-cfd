...
ax.set_ylabel('Mean Age $\\tau_{LMA}$ (s)')
ax.set_ylim(0, 52)
ax.grid(axis='y', linestyle=':', alpha=0.6)
for b, val in zip(bars, lma):
    ax.text(b.get_x() + b.get_width()/2, val + 1.2, f'{val:.1f}s', ha='center', va='bottom', fontsize=5.8, fontweight='bold')
...
c_veg_low = 0.472 * np.exp(-t / 3.5) + 0.528 * np.exp(-t / 45.0)
ax.plot(t, c_aph_nom, label='APH Nom ($\tau = 11.4$s)', color='#005696', lw=1.8)
ax.plot(t, c_aph_high, label='APH High ($\tau = 4.5$s)', color='#313695', lw=1.6)
ax.plot(t, c_veg_high, label='VEGGIE High ($\tau = 1.6$s)', color='#008080', lw=1.6)
ax.plot(t, c_veg_low, label='VEGGIE Low (53% Stagnant)', color='#d73027', lw=1.8, linestyle='--')
ax.set_xlabel('Time $t$ (s)')
ax.set_ylabel('Normalized Tracer $C(t)/C_0$')
...
ax.axhline(50.0, color='gray', linestyle='--', label='Ideal Piston (50%)')
ax.set_xticks(range(5))
ax.set_xticklabels(sys, rotation=20, ha='right', fontsize=6.5)
ax.set_ylabel('Exchange Efficiency $\epsilon_a$ (%)')
ax.set_ylim(0, 65)
ax.grid(axis='y', linestyle=':', alpha=0.6)
ax.legend(loc='upper left', framealpha=0.9)
for b, val in zip(bars_e, eff):
    ax.text(b.get_x() + b.get_width()/2, val + 1.5, f'{val:.1f}%', ha='center', va='bottom', fontsize=5.8, fontweight='bold')
...
ax.set_xticks(range(5))
ax.set_xticklabels(sys, rotation=20, ha='right', fontsize=6.5)
ax.set_ylabel('Clearance Half-Life $t_{50}$ (s)')
ax.set_ylim(0, 36)
ax.grid(axis='y', linestyle=':', alpha=0.6)
for b, val in zip(bars_t, t50):
    ax.text(b.get_x() + b.get_width()/2, val + 0.8, f'{val:.1f}s', ha='center', va='bottom', fontsize=5.8, fontweight='bold')
