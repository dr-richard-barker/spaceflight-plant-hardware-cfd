...
ax.plot(t, c_veg_high, label='VEGGIE High (Cabin Export)', color='#d73027', lw=1.6)
...
ax.set_ylabel('Normalized Bioaerosols $N(t)/N_0$')
ax.grid(True, linestyle=':', alpha=0.6)
ax.legend(loc='upper right', framealpha=0.9)

# Panel b: Spore Export vs Containment
ax = axs[0, 1]
ax.set_title('b  Cabin Bioburden vs Containment', loc='left', fontweight='bold')
sys = ['VEGGIE\n(Open)', 'APH\n(Closed)', 'CHROMEX\n(Canister)', 'CARA\n(Tape)', 'BRIC-LED\n(PDFU)']
export = [100, 0, 0, 0, 0]
contain = [0, 100, 100, 100, 100]
x = np.arange(5)
w = 0.35
ax.bar(x - w/2, export, width=w, color='#d73027', label='Cabin Export (%)')
ax.bar(x + w/2, contain, width=w, color='#1a9850', label='Containment (%)')
ax.set_xticks(x)
...
ax.set_xticklabels(sys, rotation=15, ha='right', fontsize=6.5)
ax.set_ylabel('Vulnerability Index (0-100)')
ax.set_ylim(0, 110)
ax.grid(axis='y', linestyle=':', alpha=0.6)
